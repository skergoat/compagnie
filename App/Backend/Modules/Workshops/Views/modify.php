...
<section class="section padding-responsive" id="tentative">
	...
	<div class="container">
		...
		<div class="tabs is-toggle">
		  <ul>
		    <li>
		      <a href="#" class="published <?php if($workshops->Published() == true) { ?> true <?php } else { ?> false <?php } ?>" id="button-publish">
		        <span class="icon is-small"><?php if($workshops->Published() == true) { ?><i class="eye-publish fas fa-eye-slash"></i><?php } else { ?><i class="eye-publish far fa-eye" aria-hidden="true"></i><?php } ?></span>
		        <span id="title-publish"><?php if($workshops->Published() == true) { ?> Cacher <?php } else { ?> Publier <?php } ?></span>

		        <?php if($workshops->Published() == true) { ?>
		    	...
		    	<?php } else if($workshops->Published() == false) { ?>
		    ...
		    <li>
		      <a href="/workshops-<?= $workshops->Ajax_id() ?>" id="Watch-page" class="watch-class">
		        <span class="icon is-small"><i class="fas fa-search"></i></span>
		        <span>Voir</span>
		      </a>
		    </li>
		    <li>
		      <a href="/admin/workshops-delete-<?= $workshops->getId() ?>" id="delete-page" class="delete-class">
				  ...
				  <p class="panel-heading">
					...
					<div class="panel-block">
						...
						<fieldset> Quel media intégrer dans la page ? <br/><br/>
							...
							<input type="radio" name="media" value="picture" id="picture" class="mediaType <?php if($workshops->Media() == 'picture') { echo 'radio'; } ?>" <?php if($workshops->Media() == 'picture') { echo 'checked'; } ?> /> l'image à la une <a href="https://youtu.be/HnjxlG2PsA8" target="_blank"><i class="fas fa-exclamation-triangle"></i></a><br/>
							<input type="radio" name="media" value="video" id="video" class="mediaType <?php if($workshops->Media() == 'video') { echo 'radio'; } ?>" <?php if($workshops->Media() == 'video') { echo 'checked'; } ?> /> une video <br/> 
							<input type="radio" name="media" value="slider" id="slider" class="mediaType <?php if($workshops->Media() == 'slider') { echo 'radio'; } ?>" <?php if($workshops->Media() == 'slider') { echo 'checked'; } ?>/> un slider <br/>
					...
					<div class="container-media-select" id="workshops"></div> 
...
<div class="info" id="<?= $workshops->getId() ?>"></div>			<!-- id of the page to load slider for ex. -->
<div class="moduleInfo" id="<?= $workshops->Media() ?>"></div>	<!-- to load the right media -->
